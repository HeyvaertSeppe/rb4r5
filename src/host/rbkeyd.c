 ...
 *
 * The optional fallback when the DDJ-FLX4 is not plugged in, and the handiest
 * way to drive the engine over SSH (see `rbkeyd send` below).  The player owns
 * the framebuffer and there is no X/Wayland, so this is a headless evdev
 * reader rather than a window.  It normalises key events
 * and injects them through the two FIFOs keyshim.so already consumes inside
 * rbp - exactly the same path the DDJ-FLX4 bridge uses:
 *
 ...
 * unplugged, so a wireless dongle plugged in after boot just works.  Devices
 * that are not keyboards are ignored - notably the DDJ-FLX4, which exposes
 * HID without the keys this looks for.
 *
 * Usage:
