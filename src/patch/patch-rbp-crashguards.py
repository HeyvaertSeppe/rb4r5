#!/usr/bin/env python3
"""patch-rbp-crashguards.py - the crash guard the patched `rbp` needs here.

The PrimeBox patch set (`rbp-audio`, md5 3706c68f…) turns the stock XDJ-RX3
`rbp` into a working player on non-Pioneer hardware, but it still SIGSEGVs
~1 s after start in the JUCE `NetworkMonitor` timer (seen on both the Chromebit
and the phone port, and it is not hardware-specific):
...
workaround that we must **not** apply:
...
`Ui_CycleTask` stat poll (rb2go docs/04).  Here the stock call is fine, and
forcing `getTotalLength()` to the sentinel breaks the player:
...
Usage:
    ./patch-rbp-crashguards.py --in rbp-audio --out rbp-pi5
    ./patch-rbp-crashguards.py --check rbp-pi5
"""
import argparse
import hashlib
import struct
...
def main():
    ...
    ap.add_argument("--with-length-quirk", action="store_true",
                    help="also apply the phone getTotalLength workaround "
                         "(NOT wanted here - it stops playback)")
    args = ap.parse_args()
