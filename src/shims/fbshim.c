/* fbshim.c - LD_PRELOAD ioctl shim: make rb/DirectFB see the XDJ-RX3 panel.
 *
 * rb must believe the framebuffer is the RX3's 1280x800 RGB565 panel, whatever
 * the real Pi 5 HDMI mode is (the patched DirectFB fbdev driver reads the real
 * geometry with a raw ioctl and scales/converts into it - see docs/04-display.md).
 *
 * Port of the Prime GO fbshim16 via the Chromebit port (unchanged approach):
 ...
 *
 * Note the raw syscall number: __NR_ioctl is 54 on ARM EABI (2 is fork()).
 */
#define _GNU_SOURCE
#include <stdarg.h>
#include <stddef.h>
